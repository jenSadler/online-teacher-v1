	...
	function login_redirect($redirect_url,$request_url,$user){
		...
		if(($user instanceof WP_User)){
			...
			$redirect_array = apply_filters('wplms_redirect_location',array(
					...
					'mycourses' => untrailingslashit(bp_core_get_user_domain($user->ID)).'/'.BP_COURSE_SLUG.'/',
					'instructing_courses' => untrailingslashit(bp_core_get_user_domain($user->ID)).'/'.BP_COURSE_SLUG.'/'.BP_COURSE_INSTRUCTOR_SLUG.'/',
					'dashboard' => untrailingslashit(bp_core_get_user_domain($user->ID)).'/'.(defined('WPLMS_DASHBOARD_SLUG')?WPLMS_DASHBOARD_SLUG:'dashboard').'/',
					'same' => '',
					));
