...
jQuery(document).ready(function($) {
    var vibes = {
    	...
    	children: function()
    	{
    		...
    		// remove button
    		$('body').delegate('.child-clone-row-remove','click', function() {
    			var	btn = $(this),
    				row = btn.parent();
    	...
    	{  
    		...
    		$('body').delegate('.vibe-cinput','change', function() {
    			vibes.cLoadVals();
    		});

            $('body').delegate(form,'change', function() {
                vibes.cLoadVals();
            });
    ...
    // run
    //$('#vibe-popup').livequery( function() {console.log('#############');vibes.load();} );
    
    jQuery('body').on('vibe_button_option_clicked',function(event,data){
        tb_show("Insert Shortcode", data.url + "/popup.php?popup=" + data.popup + "&width=" + 800);
    });
    document.addEventListener('vibe_shortcodes_popup_loaded',({detail})=>{
        setTimeout(function(){vibes.load();},200);
        
    });
    $('body').delegate('.the-icons i','click', function() {
    ...
    $('body').delegate('.popup-colorpicker','click', function() {           
    ...
    $('body').delegate('#vibe_type.vibe-cinput','change', function() {
