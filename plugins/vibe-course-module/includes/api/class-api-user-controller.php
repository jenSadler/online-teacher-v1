	...
	class BP_Course_Rest_User_Controller extends BP_Course_Rest_Controller {
		...
		function get_course_status_item($request){
			...
			if($item->post_type == 'unit'){
				$item->post_content = apply_filters('vibe_wplms_api_unit_content',$item->post_content,$item);
				...
				if($fetch_item){
					...
					if(($unit_type == 'play' || $unit_type == 'music-file-1') && ( false !== strpos( $item->post_content, '[' ))){
						...
						//for wplms vibezoom 
						preg_match_all( '/\[vibe_zoom token=\'(\d+)\'\]\[\/vibe_zoom]/', $item->post_content, $matches,PREG_SET_ORDER);
						if ( !empty( $matches ) && class_exists('Vibe_Zoom') ){
							foreach($matches as $key=>$shortcode2){
								if(!empty($shortcode2[1])){  // meeting_id/token
									$meeting_id = $shortcode2[1];

									$vibe_zoom_obj = Vibe_Zoom::init();
									$meeting = $vibe_zoom_obj->get_meeting($meeting_id);
									$user_id = $this->user_id;
									$users = $vibe_zoom_obj->users_from_restriction( $meeting,1);
									...
									if(!empty($meeting ) && !empty($users) && in_array($user_id , $users)){
										...
										if($meeting_expiry_time <= time()){
											$format = get_option( 'date_format' ).' '.get_option('time_format');;
											$display_expire_time = date_i18n($format ,$meeting_expiry_time_gmt );
											$iframes[] = array('shortcode'=>'vibe_zoom','status'=>0,'message'=>__('Zoom meeting expired','vibe'));
										}else{
											//$utc_time is current time adjusted with gmt
											if( $start_time < time()){ //show meeting iframe  
												$check_author_admin_for_meeting = 0;
												if($meeting_post->post_author == $user_id || is_super_admin($user_id)){
													$check_author_admin_for_meeting = 1;
												}
												$vibezoom_joinURL = '';
												...
												$iframes[] = array('shortcode'=>'vibe_zoom','status'=>1,'message'=>__('Zoom meeting found','vibe'),'value'=>$vibezoom_joinURL); 
											}else{ //show conutown 
												$remaining_time = $start_time-time();
												$iframes[] = array('shortcode'=>'vibe_zoom','status'=>0,'message'=>__('Meeting not started','vibe'),'remaining_time'=>$remaining_time); 
											}
										}
									}else{
										$iframes[] = array('shortcode'=>'vibe_zoom','status'=>0,'value'=>__('Zoom meeting can not be accessed','vibe'));
									}
								}	
							}
						}
						// remove  wplms vibe_zoom  shortcode
						$item->post_content = preg_replace ( '/\[vibe_zoom token=\'(\d+)\'\]\[\/vibe_zoom]/' , '' , $item->post_content );
