          ...
          foreach($questions['ques'] as $key=>$question){ 
            ...
            echo '<li>
              ...
              switch($type){
                case 'truefalse': 
                  ...
                  if(is_numeric($user_marked_answer)){
                    $user_marked_answer = $options[$user_marked_answer];
                  }else{$user_marked_answer='';}
                ...
                case 'survey': 
                  ...
                  if(is_numeric($user_marked_answer)){
                    $user_marked_answer =apply_filters('the_content',$options[(intval($user_marked_answer)-1)]); // Reseting for the array
                  }else{$user_marked_answer='';}
                ...
                case 'multiple': 
                      ...
                      foreach($ans as $an){
                        $user_marked_answer .= apply_filters('the_content',$options[intval($an)-1]).' ';
                      }
                  ...
                  case 'select':
                    ...
                    if(strpos($correct_answer, '|') !== false){
                      ...
                      if(strpos($user_marked_answer, '|') !== false){
                        ...
                        $user_marked_answer =implode(' '.$and.' ', $um_answers);
                      }
                    }else{
                      $user_marked_answer =apply_filters('the_content',$options[(intval($user_marked_answer)-1)]); // Reseting for the array
                      if(isset($correct_answer) && $correct_answer !=''){
              ...
              //MANISH CODES ENDS HERE;               
                  ...
                  $user_marked_answer = apply_filters('the_content',$user_marked_answer);
                  $ans = $correct_answer;
                break;
                case 'smalltext': 
                  $user_marked_answer = apply_filters('the_content',$user_marked_answer);
    ...
    function cpt_stats_query_results(){
      ...
      switch($type){
        ...
        case 'wplms-assignment':
          ...
          $maximum = $wpdb->get_results($wpdb->prepare("SELECT max(CAST(meta_value AS DECIMAL(10,0))) as max FROM {$wpdb->postmeta} WHERE meta_key REGEXP '^-?[0-9]*(\.\[0-9]*)?$' AND post_id = %d",$id));
          $minimum = $wpdb->get_results($wpdb->prepare("SELECT min(CAST(meta_value AS DECIMAL(10,0))) as min FROM {$wpdb->postmeta} WHERE meta_key REGEXP '^-?[0-9]*(\.\[0-9]*)?$' AND post_id = %d",$id));
          $avg = $wpdb->get_results($wpdb->prepare("SELECT avg(CAST(meta_value AS DECIMAL(10))) as avg FROM {$wpdb->postmeta} WHERE meta_key REGEXP '^-?[0-9]*(\.\[0-9]*)?$'' AND post_id = %d",$id));
